...
namespace Binance.API.Csharp.Client
{
    public class ApiClient : ApiClientAbstract, IApiClient
    {
        ...
        /// <returns></returns>
        public async Task<string> CallAsync<T>(ApiMethod method, string endpoint, bool isSigned = false, string parameters = null)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                // Get the result
                var x = response.Content.ReadAsStringAsync().Result;//.ConfigureAwait(false);
                return x;
